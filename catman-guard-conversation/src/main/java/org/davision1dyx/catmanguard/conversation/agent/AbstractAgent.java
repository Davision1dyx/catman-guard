...
public abstract class AbstractAgent {
    ...
    protected ContextSessionService contextSessionService;
    ...
    public AbstractAgent(String name, ChatModel chatModel, String agentType) {
        this.name = name;
     ...
     * @return
     */
    public abstract Flux<String> execute(String question, String conversationId);
    ...
    public ChatMemory createPersistentChatMemory(String sessionId, int maxMessages) {
        ...
        // 查询数据库中的对话历史
        List<ContextSessionVO> history = contextSessionService.findRecentBySessionId(sessionId, maxMessages);
        ...
        if (history != null && !history.isEmpty()) {
            ...
            for (int i = history.size() - 1; i >= 0; i--) {
                ContextSessionVO record = history.get(i);
                // 添加用户问题
    ...
    public AgentTaskManager.TaskInfo registerTask(String conversationId, Sinks.Many<String> sink) {
    ...
    protected String generateRecommendations(String conversationId, String currentQuestion, String currentAnswer) {
        ...
        try {
            ...
            // 1. 添加系统提示词
            messages.add(new SystemMessage(CommonPrompt.RECOMMEND_QUESTION_PROMPT));
            ...
            try {
                ...
                log.error("调用模型生成推荐问题时发生超时或网络错误", e);
                throw e; // 重新抛出异常，由外层 catch 处理
            }
